...
def Track(images: np.ndarray, costOfNonAssignment: float, costFunction: CostFunctionType,
    ...
    for i in range(images.shape[0]):
        ...
        mapping = np.asarray(UpdateTracks(tracks, images[i], costOfNonAssignment, trackLostCutoff, costFunction))

        # Check if mapping is empty or not 2-dimensional
        if mapping.ndim != 2 or mapping.size == 0:
            # Handle the case where there are no organoids to track
            # For example, you can just copy the original image or set it to zeros
            relabeledImages[i] = np.zeros_like(images[i])
        else:
